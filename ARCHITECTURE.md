...
stateDiagram-v2
  ...
  ANSWERING --> DISCUSSING: 全回答者の提出後にホストが公開
...
最後の提出を保存してもANSWERINGを維持し、回答本文は公開しない。回答数と固定された回答者数が一致した後、ホストの`round.publish`を別のトランザクションで確定してDISCUSSINGへ変更し、commit後に全員へ一斉公開する。公開コマンドでも回答数を再検査するため、直前に取消が成立していれば公開しない。Figmaの5秒待ちのデモ遷移は実装しない。

取消はANSWERING中だけ。公開が先にcommitした場合、後から届く取消はPHASE_CHANGEDになる。取消が先なら未提出者が残るため公開コマンドをINCOMPLETEで拒否する。クライアント時刻による順序変更や公開後の巻き戻しは行わない。
